...
class ProjectBase
{
    ...
    const REDIS_PREFIX_SESSION = 'sy' . SY_PROJECT . '000001_'; //前缀-session
    const REDIS_PREFIX_SESSION_LIST = 'sy' . SY_PROJECT . '000002_'; //前缀-session列表
    const REDIS_PREFIX_SESSION_JWT_REFRESH = 'sy' . SY_PROJECT . '00003_'; //前缀-会话JWT更新
    const REDIS_PREFIX_CODE_IMAGE = 'sy' . SY_PROJECT . '001000_'; //前缀-验证码图片
    const REDIS_PREFIX_UNIQUE_ID = 'sy' . SY_PROJECT . '001001_'; //前缀-唯一ID
    const REDIS_PREFIX_MESSAGE_QUEUE = 'sy' . SY_PROJECT . '001002_'; //前缀-消息队列
    const REDIS_PREFIX_IMAGE_DATA = 'sy' . SY_PROJECT . '001003_'; //前缀-图片缓存
    const REDIS_PREFIX_IM_ADMIN = 'sy' . SY_PROJECT . '001004_'; //前缀-im管理账号缓存
    const REDIS_PREFIX_KAFKA_MESSAGE_OFFSET = 'sy' . SY_PROJECT . '001005_'; //前缀-kafka消息位移缓存
    const REDIS_PREFIX_PAY_HASH = 'sy' . SY_PROJECT . '001006_'; //前缀-支付哈希
    const REDIS_PREFIX_WX_ACCOUNT = 'sy' . SY_PROJECT . '002000_'; //前缀-微信公众号
    const REDIS_PREFIX_WX_COMPONENT_ACCOUNT = 'sy' . SY_PROJECT . '002001_'; //前缀-微信开放平台账号
    const REDIS_PREFIX_WX_COMPONENT_AUTHORIZER = 'sy' . SY_PROJECT . '002002_'; //前缀-微信开放平台授权公众号
    const REDIS_PREFIX_WX_NATIVE_PRE = 'sy' . SY_PROJECT . '002003_'; //前缀-微信扫码预支付
    const REDIS_PREFIX_WX_PROVIDER_CORP_ACCOUNT = 'sy' . SY_PROJECT . '002004_'; //前缀-企业微信服务商账号
    const REDIS_PREFIX_WX_PROVIDER_CORP_ACCOUNT_SUITE = 'sy' . SY_PROJECT . '002005_'; //前缀-企业微信服务商套件
    const REDIS_PREFIX_WX_PROVIDER_CORP_AUTHORIZER = 'sy' . SY_PROJECT . '002006_'; //前缀-服务商授权企业微信
    const REDIS_PREFIX_WX_CORP = 'sy' . SY_PROJECT . '002007_'; //前缀-企业微信
    const REDIS_PREFIX_WX_COMPONENT_AUTHORIZER_CODE_SECRET = 'sy' . SY_PROJECT . '002008_'; //前缀-微信开放平台授权小程序代码保护密钥
    const REDIS_PREFIX_TIMER_QUEUE = 'sy' . SY_PROJECT . '003000_'; //前缀-定时器队列
    const REDIS_PREFIX_TIMER_CONTENT = 'sy' . SY_PROJECT . '003001_'; //前缀-定时器内容
    const REDIS_PREFIX_PRINT_FEYIN_ACCOUNT = 'sy' . SY_PROJECT . '004000_'; //前缀-飞印打印账号
    const REDIS_PREFIX_DINGTALK_PROVIDER_ACCOUNT = 'sy' . SY_PROJECT . '005000_'; //前缀-企业钉钉服务商账号
    const REDIS_PREFIX_DINGTALK_PROVIDER_ACCOUNT_SUITE = 'sy' . SY_PROJECT . '005001_'; //前缀-企业钉钉服务商套件
    const REDIS_PREFIX_DINGTALK_PROVIDER_AUTHORIZER = 'sy' . SY_PROJECT . '005002_'; //前缀-服务商授权企业钉钉
    const REDIS_PREFIX_DINGTALK_CORP = 'sy' . SY_PROJECT . '005003_'; //前缀-企业钉钉
    const REDIS_PREFIX_JPUSH_APP_CID_PUSH = 'sy' . SY_PROJECT . '006000_'; //前缀-极光推送推送唯一标识符
    const REDIS_PREFIX_JPUSH_APP_CID_SCHEDULE = 'sy' . SY_PROJECT . '006001_'; //前缀-极光推送定时任务唯一标识符
    const REDIS_PREFIX_CODE_WEBHOOK_QUEUE = 'sy' . SY_PROJECT . '007000_'; //前缀-代码WebHook队列
    const REDIS_PREFIX_CODE_WEBHOOK_STATUS = 'sy' . SY_PROJECT . '007001_'; //前缀-代码WebHook状态
    const REDIS_PREFIX_CODE_WEBHOOK_COMMAND = 'sy' . SY_PROJECT . '007002_'; //前缀-代码WebHook命令
    const REDIS_PREFIX_CODE_WEBHOOK_INFO = 'sy' . SY_PROJECT . '007003_'; //前缀-代码WebHook信息
    const REDIS_PREFIX_DUPLICATE_REQUEST = 'sy' . SY_PROJECT . '008001_'; //前缀-重复请求缓存
    ...
    const TIME_EXPIRE_SWOOLE_CLIENT_HTTP = 20000; //超时时间-http服务客户端,单位为毫秒
    const TIME_EXPIRE_SWOOLE_CLIENT_RPC = 20000; //超时时间-rpc服务客户端,单位为毫秒
    ...
    const TIME_EXPIRE_LOCAL_PRINT_REFRESH = 600; //超时时间-本地打印机配置更新,单位为秒
    const TIME_EXPIRE_LOCAL_PRINT_CLEAR = 3600; //超时时间-本地打印机配置清理,单位为秒
    ...
    //容量常量
    const SIZE_SERVER_PACKAGE_MAX = 20971520; //服务端容量-最大接收数据大小,单位为字节,默认为20M
    ...
    //打印机常量
    const PRINT_CONFIG_BASE_STATUS_ENABLE = 1; //状态-有效
    const PRINT_CONFIG_BASE_STATUS_DISABLE = 2; //状态-无效
    const PRINT_CONFIG_BASE_TYPE_ENABLE = 1; //类型-自动打印
    const PRINT_CONFIG_BASE_TYPE_DISABLE = 2; //类型-手动打印
}
