...
class MediaUpload extends WxBaseShop
{
    ...
    public function setFilePath(string $filePath)
    {
        if (file_exists($filePath) && is_readable($filePath)) {
            if(class_exists('\CURLFile')){
                $this->reqData['media'] = new \CURLFile(realpath($filePath));
            }else{
                $this->reqData['media'] = '@'. $filePath;
            }
        } else {
    ...
    public function getDetail() : array
    {
        ...
        $this->curlConfigs[CURLOPT_POSTFIELDS] = $this->reqData['media'];
        $sendRes = WxUtilBase::sendPostReq($this->curlConfigs);
        $sendData = Tool::jsonDecode($sendRes);
        Log::log('send_data_res3333:' . print_r($sendData,true));
