...
class OrderRefund extends WxBaseShop
{
    ...
    public function getDetail() : array
    {
        ...
        if ($this->merchantType == self::MERCHANT_TYPE_SELF) {
            ...
            $spInfo = WxConfigSingleton::getInstance()->getWxSpConfig($appId);
            $tmpKey = tmpfile();
            fwrite($tmpKey, $spInfo['api_key']);
            $tmpKeyData = stream_get_meta_data($tmpKey);
            $tmpCert = tmpfile();
            fwrite($tmpCert, $spInfo['api_cert']);
            $tmpCertData = stream_get_meta_data($tmpCert);
        }
        $this->reqData['sign'] = WxUtilShop::createSign($this->reqData, $appId, $this->merchantType);
        ...
        Log::log('REFUND_POST:' . print_r($this->curlConfigs,true));
        $sendRes = WxUtilBase::sendPostReq($this->curlConfigs);
        fclose($tmpKey);
        fclose($tmpCert);
        Log::log('REFUNDRes:' . $sendRes);
