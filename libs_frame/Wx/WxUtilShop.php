...
final class WxUtilShop extends WxUtilBaseAlone
{
     ...
     * @return string
     */
    public static function createSign(array $data, string $appId, string $merchantType = 'self', string $signType = 'md5')
    {
        //签名步骤一：按字典序排序参数
        ksort($data);
        //签名步骤二：格式化后加入KEY
        ...
        if ($merchantType == WxBaseShop::MERCHANT_TYPE_SELF) {
            $payKey = WxConfigSingleton::getInstance()->getShopConfig($appId)->getPayKey();
        }else{
            $spInfo = WxConfigSingleton::getInstance()->getWxSpConfig($appId); //获取服务商微信系统配置api秘钥
            $payKey = $spInfo['pay_key'];
        }
     ...
     * @return bool
     */
    public static function checkSign(array $data, string $appId, string $merchantType) : bool
    {
        if (isset($data['sign']) && is_string($data['sign'])) {
            $nowSign = self::createSign($data, $appId, $merchantType);
