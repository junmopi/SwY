...
class FansList extends DouYinBaseOpen {
    ...
    {
        $this->curlConfigs[CURLOPT_URL] = $this->serviceDomain . '/fans/list?' . http_build_query($this->reqData);
        $sendRes = DouYinUtilBase::sendGetReq($this->curlConfigs);
        $sendData = Tool::jsonDecode($sendRes);
        if ($sendData['data']['error_code'] > 0) {
            Log::log('DouYinFansList:' . $sendData['data']['description']);
